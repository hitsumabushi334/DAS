...
class WorkflowRealApiTestSuite {
  ...
  testBasicWorkflowFunctionality() {
    ...
    this.framework.runTest("初期化エラーハンドリング", () => {
      try {
        ...
        // デフォルト値が設定されていることを確認
        this.framework.assertNotNull(
          invalidWorkflow.features,
          "featuresにデフォルト値が設定されている",
        );
        this.framework.assertNotNull(
        ...
        // デフォルト値の内容確認
        this.framework.assertEqual(
          invalidWorkflow.features.speechToText,
          false,
          "speechToTextのデフォルト値",
        );
        this.framework.assertEqual(
          invalidWorkflow.features.textToSpeech,
          false,
          "textToSpeechのデフォルト値",
        );
        this.framework.assertEqual(
          invalidWorkflow.features.fileUpload,
          false,
          "fileUploadのデフォルト値",
        );
        this.framework.assertEqual(
          invalidWorkflow.features.suggestedQuestionsAfterAnswer,
          false,
          "suggestedQuestionsAfterAnswerのデフォルト値",
        );
  ...
  testApplicationSettings() {
    ...
    // 4. fileUploadプロパティ詳細検証
    this.framework.runTest("fileUpload - プロパティ詳細検証", () => {
      try {
        ...
        Logger.log(`fileUpload設定が正常に初期化されました`);
      } catch (error) {
        Logger.log(`fileUploadプロパティ検証エラー: ${error.message}`);
        this.framework.assertTrue(
          error.message.includes("API エラー"),
          "APIエラー",
        );
